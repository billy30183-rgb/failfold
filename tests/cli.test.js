...
test('CLI treats --help after option terminator as an input filename',t=>{const p=env(t);fs.writeFileSync(path.join(p,'--help'),xml(bad));const r=run(['--','--help'],p);assert.equal(r.status,0,r.stderr);assert.match(r.stdout,/# FailFold report/);});
...
test('CLI refuses a dangling symlink output even with force',t=>{const p=env(t),link=path.join(p,'r.md'),target=path.join(p,'created-through-link.md');let fileLink=true;try{fs.symlinkSync(path.basename(target),link,'file');}catch(e){if(!['EPERM','EACCES','ENOTSUP'].includes(e.code))throw e;fileLink=false;const dir=path.join(p,'junction-target');fs.mkdirSync(dir);try{fs.symlinkSync(dir,link,'junction');fs.rmSync(dir,{recursive:true});}catch(junctionError){t.skip(`symlink unavailable: ${junctionError.code}`);return;}}const r=run(['now.xml','--output','r.md','--force'],p);assert.equal(r.status,2,r.stderr);assert.match(r.stderr,/Refusing symlink output/);if(fileLink)assert.equal(fs.existsSync(target),false);assert.equal(fs.lstatSync(link).isSymbolicLink(),true);});
